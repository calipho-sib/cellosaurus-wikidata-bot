...
'''
Updates the Wikidata items relative to the cells in the Cellosaurus 
dump of interest, but only for some cell lines.
...
- 5th: A txt file with one cell_line Cellosaurus ID per line   
...
def main():
    ...
    lines_to_update_path = sys.argv[5]
    assert lines_to_update_path, "You need to provide a file with lines to update"
    ...
    text_file = open(lines_to_update_path, "r")
    lines_to_update = text_file.read().split("\n")
    for cellosaurus_id in tqdm(lines_to_update):
